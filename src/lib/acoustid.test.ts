...
Deno.test("Acoustid Tests", async (t) => {
  ...
  await t.step("generateFingerprint", async (tInner) => {
    ...
    await tInner.step("should return null if JSON is valid but fingerprint is missing", async () => {
      const { generateFingerprint } = await import("./acoustid.ts");
      MockDenoCommand.addMock("fpcalc", {
        code: 0,
        stdout: JSON.stringify({
  ...
  await t.step("processAcoustIDTagging", async (tInner) => {
    await tInner.step("should handle file not found", async () => {
      ...
      const statStub = stub(Deno, "stat", returnsNext([
    ...
    await tInner.step("should handle fingerprint generation failure", async () => {
      const { processAcoustIDTagging } = await import("./acoustid.ts");
      const statStub = stub(Deno, "stat", returnsNext([
      ...
      const readFileStub = stub(Deno, "readFile", returnsNext([
      ...
      try {
        ...
        assert(logCalls.some(msg => msg.includes("WARNING: Could not generate fingerprint")));
      } finally {
        statStub.restore();
