...
import { ensureDir, copy, exists } from "std/fs/mod.ts";
...
async function setupTestFiles(testRunDir: string, fileNames: string[]): Promise<string[]> {
  ...
  for (const fileName of fileNames) {
    ...
    if (!await exists(sourcePath, { isFile: true })) {
      throw new Error(`Source sample file not found: ${sourcePath}. Ensure sample files are present.`);
    }
...
async function runAmusicScript(args: string[], cwd: string): Promise<AmusicRunResult> {
...
async function getSpecificTag(filePath: string, tagName: string): Promise<string | null> {
  ...
  if (code !== 0) {
    ...
    if (errorOutput && !errorOutput.includes("does not contain any stream") && !errorOutput.includes(filePath)) {
      console.warn(`  ffprobe warning for ${filePath} checking tag ${tagName}: ${errorOutput.split("\n")[0]}`);
    }
    return null;
  }
...
const getAcousticIDFingerprintTag = (filePath: string) => getSpecificTag(filePath, "ACOUSTID_FINGERPRINT");
...
Deno.test("amusic.ts Integration Tests (Actual Audio Files)", async (t) => {
  ...
  await t.step("Basic Fingerprinting: Process a clean MP3 file", async () => {
    ...
    if (initialTag) {
        console.warn(`  Warning: Sample file ${mp3BaseName} already had an ACOUSTID_FINGERPRINT. Test will proceed assuming overwrite is fine.`);
    }
    ...
    // 3. Assertions
    assertEquals(result.code, 0, "Script should exit successfully after processing.");
    assertStringIncludes(result.stdout, `Processing file: ${mp3BaseName}`);
    assertStringIncludes(result.stdout, "ACTION: Generating AcoustID fingerprint...");
    assertStringIncludes(result.stdout, "SUCCESS: AcoustID fingerprint tag processed.");

    const finalTag = await getAcousticIDFingerprintTag(mp3ToTest);
    assertExists(finalTag, "ACOUSTID_FINGERPRINT tag should be present after processing.");
    if (initialTag) {
        assertNotEquals(finalTag, initialTag, "If an initial tag existed, the new tag should be different (or fpcalc is very stable). This is a soft check.");
    }
  ...
  await t.step("Skip Existing Fingerprint (No --force): MP3 file", async () => {
    ...
    assertExists(firstFingerprint, "Fingerprint not added after initial processing.");

    // 2. Run amusic.ts again without --force
    runResult = await runAmusicScript([mp3BaseName], currentTestDir);

    // 3. Assertions
    assertEquals(runResult.code, 0, "Script should exit successfully when skipping.");
    assertStringIncludes(runResult.stdout, `Processing file: ${mp3BaseName}`);
    assertStringIncludes(runResult.stdout, "INFO: File already has AcoustID tags. Skipping (use --force to overwrite).");

    const secondFingerprint = await getAcousticIDFingerprintTag(mp3ToTest);
    assertEquals(secondFingerprint, firstFingerprint, "Fingerprint should remain unchanged after skipping.");

    await cleanupTestDir(currentTestDir);
  });

  await t.step("Force Overwrite Existing Fingerprint (--force): FLAC file", async () => {
    ...
    assertEquals(runResult.code, 0, "Initial processing for force test failed.");
    const firstFingerprint = await getAcousticIDFingerprintTag(flacToTest);
    assertExists(firstFingerprint, "Fingerprint not added after initial processing for force test.");
    ...
    runResult = await runAmusicScript(["--force", flacBaseName], currentTestDir);

    // 3. Assertions
    assertEquals(runResult.code, 0, "Script should exit successfully during force overwrite.");
    assertStringIncludes(runResult.stdout, `Processing file: ${flacBaseName}`);
    assertStringIncludes(runResult.stdout, "INFO: File already has AcoustID tags. --force option provided, proceeding to overwrite.");
    assertStringIncludes(runResult.stdout, "ACTION: Generating AcoustID fingerprint...");
    assertStringIncludes(runResult.stdout, "SUCCESS: AcoustID fingerprint tag processed.");

    const secondFingerprint = await getAcousticIDFingerprintTag(flacToTest);
    assertExists(secondFingerprint, "Fingerprint should exist after force overwrite.");
    ...
    assertEquals(secondFingerprint, firstFingerprint, "Fingerprint should ideally be the same if fpcalc is deterministic.");
  ...
  await t.step("File Not Found", async () => {
    ...
    assertStringIncludes(result.stdout, `Error: File not found at "${nonExistentFileName}".`);
    // if (result.code === 0) {
  ...
  await t.step("Processing Multiple Files (MP3 clean, FLAC pre-tagged)", async () => {
    currentTestDir = await createTestRunDir("multiple_files");
    const [mp3File, flacFile] = await setupTestFiles(currentTestDir, [SAMPLE_MP3, SAMPLE_FLAC]);
    ...
    assertEquals(flacResult.code, 0, "Initial processing of FLAC file failed.");
    ...
    if (mp3InitialTag) {
      ...
      throw new Error(`Test setup error: MP3 file ${mp3BaseName} was not clean before multiple file test.`);
    }

    // 3. Run amusic.ts on both files
    const mainResult = await runAmusicScript([mp3BaseName, flacBaseName], currentTestDir);
    assertEquals(mainResult.code, 0, "Main script run for multiple files failed.");

    // 4. Assertions for MP3 (should be processed)
    assertStringIncludes(mainResult.stdout, `Processing file: ${mp3BaseName}`);
    assertStringIncludes(mainResult.stdout, `SUCCESS: AcoustID fingerprint tag processed.`);
    const mp3FinalTag = await getAcousticIDFingerprintTag(mp3File);
    assertExists(mp3FinalTag, "MP3 file should have a fingerprint after multi-file run.");

    // 5. Assertions for FLAC (should be skipped)
    assertStringIncludes(mainResult.stdout, `Processing file: ${flacBaseName}`);
    assertStringIncludes(mainResult.stdout, `INFO: File already has AcoustID tags. Skipping (use --force to overwrite).`);
    const flacFinalTag = await getAcousticIDFingerprintTag(flacFile);
    assertEquals(flacFinalTag, flacInitialTag, "FLAC file's fingerprint should be unchanged.");
